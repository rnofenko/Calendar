...
namespace Bs.Calendar.DataAccess
{
    public class UserRepository : BaseRepository<User>, IUserRepository
    {
        ...
        {
            var query = Load()
                .WhereIf(filter.Email.IsNotEmpty(), x => x.Email.Contains(filter.Email, StringComparison.OrdinalIgnoreCase))
                .WhereIf(filter.Name.IsNotEmpty(), x => x.FullName.Contains(filter.Name, StringComparison.OrdinalIgnoreCase))
                .WhereIf(filter.SecondName.IsNotEmpty(), x => x.FullName.Contains(filter.SecondName, StringComparison.OrdinalIgnoreCase))
